...
def remove_redundant_edges(graph):
    ...
    for edge in list(graph.edges):
        child, parent = edge
        # 检查是否存在从 child 到 parent 的间接路径（不包含直接边 child -> parent）
        if nx.has_path(graph, child, parent):
            # 如果存在路径，则边 child -> parent 是冗余的
            redundant_edges.append((child, parent))
...
def main():
    ...
    new_relations = pd.read_csv("prediction_data/go_2022/fillter/ndr_pairs_with_uncle_nephew_iter_pre.csv", sep='\t', header=None,
                                names=['child', 'parent'])
